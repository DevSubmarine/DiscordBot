...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(NotDevSubBaseClassCodeFixProvider)), Shared]
    public class NotDevSubBaseClassCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds
            => ImmutableArray.Create(DiagnosticID.NotDevSubBaseClass);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: CodeFixResources.NotDevSubBaseClass_ChangeBaseClass,
                    createChangedDocument: c => this.ChangeBaseClass(context.Document, declaration, c),
                    equivalenceKey: nameof(CodeFixResources.NotDevSubBaseClass_ChangeBaseClass)),
                diagnostic);
        }
        ...
        private async Task<Document> ChangeBaseClass(Document document, TypeDeclarationSyntax declaration, CancellationToken cancellationToken)
        {
            ...
            types = types.Insert(0, SyntaxFactory.SimpleBaseType(
                SyntaxFactory.ParseTypeName(RequiredTypeName.DevSubInteractionModule)
